...
namespace MatheusProductions.RodheLib
{
    public class Rodhe
    {
        ...
        public static void SalvaPrints(FormattedIO488 instr, string nomePasta, string nomePrint, bool tPrints, string tipoImagem)
        {
            ...
            if (tPrints)
            {
                instr.WriteString(@$"HCOP:CMAP:DEF 4");
                if (!System.IO.File.Exists(nomePasta))
                {
                    ...
                    instr.WriteString(@$"MMEM:NAME 'C:\\prints.png'"); // Faça a captura de tela
                    instr.WriteString(@$"HCOP"); // Faça a captura de tela
                    nomePasta = nomePasta + @"\" + nomePrint + ".png";
                    instr.WriteString(@$"MMEM:DATA? 'C:\\prints.png'"); // Faça a captura de tela
        ...
        public static void ConfiguraInstr6(FormattedIO488 instr, string freqC, string unidadeY, string att, string refL, string span, string rbw, string vbw, string sweepAuto, string trace, string detector, string modo, string porc_Ocu, string qDbs)
        {
            //OBW
            instr.WriteString($"CALC:MARK1:FUNC:POW:SEL {modo}"); // Seleciona o modo
            instr.WriteString($"CALC:UNIT:POW {unidadeY}"); //Configura a unidade do reference Level
            instr.WriteString($"INP:ATT {att} dB"); //Configura o ATT
            instr.WriteString($"DISP:TRAC:Y:RLEV {refL} dbm"); // Configura o Reference Level
            instr.WriteString($"FREQ:CENT {freqC} Mhz"); // Configura a Frequencia Central
